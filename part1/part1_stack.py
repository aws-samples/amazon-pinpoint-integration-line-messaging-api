...
from aws_solutions_constructs.aws_cloudfront_apigateway_lambda import (
...
class Part1Stack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Initialize Bucket
        image_bucket = _s3.Bucket(
            self,
            "image_s3_bucket",
            block_public_access=_s3.BlockPublicAccess.BLOCK_ALL,
            enforce_ssl=True,
            versioned=True,
            access_control=_s3.BucketAccessControl.LOG_DELIVERY_WRITE,
            server_access_logs_prefix="access-logs",
            removal_policy=RemovalPolicy.DESTROY,
            object_ownership=_s3.ObjectOwnership.BUCKET_OWNER_PREFERRED,
        )
        audio_bucket = _s3.Bucket(
            self,
            "audio_s3_bucket",
            block_public_access=_s3.BlockPublicAccess.BLOCK_ALL,
            enforce_ssl=True,
        ...
        image_bucket_deployment = _s3_deploy.BucketDeployment(
            self,
            "image_bucket_deploy",
            destination_bucket=image_bucket,
            sources=[_s3_deploy.Source.asset("./assets/files/image_files/")],
        )
        audio_bucket_deployment = _s3_deploy.BucketDeployment(
            self,
            "audio_bucket_deploy",
            destination_bucket=audio_bucket,
            sources=[_s3_deploy.Source.asset("./assets/files/audio_files/")],
        )
        video_bucket_deployment = _s3_deploy.BucketDeployment(
            self,
            "video_bucket_deploy",
            destination_bucket=video_bucket,
            sources=[_s3_deploy.Source.asset("./assets/files/video_files/")],
        )

        # Initialize Pinpoint Project
        pinpoint_project = _pinpoint.CfnApp(
            self, "line-pinpoint-project", name="Line-Pinpoint-Project"
        )
        # Create Line credential Placeholder Secrets in AWS Secret Manager
        line_credentials = _secrets.Secret(
            self,
            "line-credentials",
            secret_name="line_secrets",
            secret_object_value={
                "YOUR_CHANNEL_ACCESS_TOKEN": SecretValue.unsafe_plain_text(
                    "INSERT_CHANNEL_TOKEN"
                ),
                "YOUR_CHANNEL_SECRET": SecretValue.unsafe_plain_text(
                    "INSERT_CHANNEL_SECRET"
                ),
            },
        )

        # Create a lambda layer that is shared between different functions
        shared_lambda_layer = _python.PythonLayerVersion(
            self,
            "shared_lambda_layer",
            entry="./assets/lambda_layers",
            compatible_runtimes=[_lambda.Runtime.PYTHON_3_9],
        )
        # Create sendText Lambda Function
        send_text_lambda = _python.PythonFunction(
            self,
            "send_text_lambda",
            entry="./assets/lambda_functions",
            runtime=_lambda.Runtime.PYTHON_3_9,
            index="sendText.py",
            handler="lambda_handler",
            description="Lambda function used by Amazon Pinpoint to send Text messages via LINE",
            layers=[shared_lambda_layer],
            environment={
        ...
        send_image_lambda = _python.PythonFunction(
            self,
            "send_image_lambda",
            entry="./assets/lambda_functions",
            runtime=_lambda.Runtime.PYTHON_3_9,
            index="sendImage.py",
            handler="lambda_handler",
            description="Lambda function used by Amazon Pinpoint to send Image messages via LINE",
            layers=[shared_lambda_layer],
            environment={
                "secret_arn": line_credentials.secret_arn,
                "secret_region": os.environ["CDK_DEFAULT_REGION"],
        ...
        send_audio_lambda = _python.PythonFunction(
            self,
            "send_audio_lambda",
            entry="./assets/lambda_functions",
            runtime=_lambda.Runtime.PYTHON_3_9,
            index="sendAudio.py",
            handler="lambda_handler",
            description="Lambda function used by Amazon Pinpoint to send Audio messages via LINE",
            layers=[shared_lambda_layer],
            environment={
                "secret_arn": line_credentials.secret_arn,
                "secret_region": os.environ["CDK_DEFAULT_REGION"],
        ...
        send_video_lambda = _python.PythonFunction(
            self,
            "send_video_lambda",
            entry="./assets/lambda_functions",
            runtime=_lambda.Runtime.PYTHON_3_9,
            index="sendVideo.py",
            handler="lambda_handler",
            description="Lambda function used by Amazon Pinpoint to send Video messages via LINE",
            layers=[shared_lambda_layer],
            environment={
                "secret_arn": line_credentials.secret_arn,
                "secret_region": os.environ["CDK_DEFAULT_REGION"],
        ...
        receive_event_lambda = _python.PythonFunction(
            self,
            "receive_event_lambda",
            entry="./assets/lambda_functions",
            runtime=_lambda.Runtime.PYTHON_3_9,
            index="receiveEvent.py",
            handler="lambda_handler",
            description="Lambda function used to receive user events from LINE",
            layers=[shared_lambda_layer],
            environment={
                "secret_arn": line_credentials.secret_arn,
                "secret_region": os.environ["CDK_DEFAULT_REGION"],
        ...
        receive_event_lambda.role.add_to_principal_policy(
        ...
        send_image_lambda.grant_invoke(
            _iam.ServicePrincipal(
                service="pinpoint.amazonaws.com",
                conditions={
                    "ArnLike": {
        ...
        send_audio_lambda.grant_invoke(
            _iam.ServicePrincipal(
                service="pinpoint.amazonaws.com",
                conditions={
                    "ArnLike": {
        ...
        send_video_lambda.grant_invoke(
            _iam.ServicePrincipal(
                service="pinpoint.amazonaws.com",
                conditions={
                    "ArnLike": {
        ...
        CloudFrontToApiGatewayToLambda(
            ...
            existing_lambda_obj=receive_event_lambda,
            cloud_front_logging_bucket_props=_s3.BucketProps(
                server_access_logs_prefix="access-logs"
            ),
        )
